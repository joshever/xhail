...
def benchmark_table():
    ...
    rows = [
        ...
        [
            ...
            "6 (4+, 2−)",
            "1",
            "mammal(V1) :- produces_milk(V1).",
            "1.070",
        ],
        ["traffic", "Rule learning", "4 (2+, 2−)", "1", "stop(V1) :- red(V1).", "0.045"],
        ["propositional", "0-arity (propositional)", "1 (1+, 0−)", "1", "output :- .", "0.008"],
    ]
...
def build():
    ...
    story += [
        ...
        p("XHAIL", "title"),
        p(
            "Re-engineering an Abductive-Inductive Learning System<br/>"
            "for Research-Grade Symbolic AI",
            "subtitle",
        ),
        sp(0.3),
        ...
        sp(0.1),
        p(
            "<b>Keywords:</b> inductive logic programming &nbsp;&#183;&nbsp; "
    ...
    story += [
        ...
        rule(),
        p(
            "The dominant paradigm in machine learning is statistical: models learn "
        ...
        p(
            "XHAIL (eXtended Hybrid Abductive Inductive Learning) was introduced "
        ...
        p(
            "Despite its theoretical elegance, XHAIL has seen limited implementation "
    ...
    story += [
        ...
        p(
            "ILP (Muggleton, 1991) learns first-order logic programs from examples. "
        ...
        p(
            "ASP is a declarative paradigm based on stable model semantics. The clingo "
    ...
    story += [
        h1("3", "System Description"),
        rule(),
        h2("3.1  Input Format"),
        p(
            "An XHAIL input file (<tt>.lp</tt>) contains background knowledge, "
        ...
        p(
            "Finds the minimal set of ground atoms &#916; that, with background "
        ...
        p(
            "Constructs the <i>kernel set</i> K — maximally-specific clauses "
        ...
        p(
            "Finds the minimal H &#8838; K covering all examples. Encoded as "
    ...
    story += [
        ...
        bullet(
            "<b>D3+D5</b> — Parser missed propositional atoms and <tt>:-</tt> "
            "integrity constraints."
        ),
        bullet("<b>D6</b> — Deduction phase hung indefinitely; configurable timeout added."),
        bullet(
            "<b>D8–D10</b> — Mutable class-level defaults, no-op set operations, "
            "incorrect <tt>replaceConstants</tt> indexing."
        ),
        bullet(
            "<b>D14</b> — Propositional trailing comma: <tt>try(0, 1, )</tt> generated "
            "for 0-arity predicates. Fixed via <tt>_try_term()</tt> helper "
            "in <tt>induction.py</tt>."
        ),
        h2("4.4  Testing and CI"),
        p(
            "101 tests span unit (language structures, parser, API contracts), "
    ...
    story += [
        ...
        rule(),
        p(
            "Table 1 compares XHAIL against the four most relevant ILP systems "
        ...
        h2("XHAIL vs Aleph"),
        p(
            "Both use mode declarations and ground-atom examples. Aleph uses "
        ...
        h2("XHAIL vs Metagol"),
        p(
            "Metagol excels at recursive program synthesis via higher-order "
        ...
        h2("XHAIL vs ILASP"),
        p(
            "ILASP is the most expressive ASP-based ILP system, learning full "
        ...
        h2("XHAIL vs FastLAS"),
        p(
            "FastLAS prioritises scalability via aggressive hypothesis-space "
    ...
    story += [
        ...
        bullet(
            "<b>penguins</b> — Learn that birds fly unless they are penguins. "
            "Tests negation-as-failure."
        ),
        bullet(
            "<b>animals</b> — Learn that mammals have hair or produce milk. "
            "Tests multi-predicate feature classification."
        ),
        bullet(
            "<b>traffic</b> — Learn that vehicles stop at red lights. "
            "Tests simple deterministic rule learning."
        ),
        bullet(
            "<b>propositional</b> — Learn a 0-arity output from 0-arity inputs. "
            "Tests the propositional code path fixed in this work."
        ),
        h2("6.2  Metrics Recorded"),
        p(
            "For each run: wall-clock time (<tt>runtime_s</tt>), peak RSS "
        ...
        sp(0.2),
        p(
            "All four benchmarks are solved correctly. Runtimes span two orders of "
    ...
    story += [
        ...
        rule(),
        p(
            "Five limitations were characterised empirically. Each is a concrete "
            "research target rather than a fundamental obstacle."
        ),
        h2("L1 — Multi-head learning"),
        p(
            "<tt>induction.py</tt> hardcodes <tt>new_head = clauses[0].head</tt>, "
        ...
        p(
            "If a modeb predicate appears in its own derivation, the deduction "
        ...
        p(
            "A <tt>-type</tt> placemarker in <tt>#modeh</tt> causes an "
        ...
        p(
            "0-arity predicates previously generated <tt>try(0, 1, )</tt> — "
        ...
        p(
            "Pathological inputs hang indefinitely. A <tt>DeductionTimeoutError</tt> "
    ...
    story += [
        ...
        h2("Near-term"),
        bullet(
            "Fix L1 (multi-head) — one-line change in <tt>runPhase()</tt>; "
            "unblocks all multi-class classification tasks."
        ),
        bullet(
            "Fix L3 (output vars in modeh) — one-line <tt>isinstance</tt> check; "
            "unblocks relational tasks."
        ),
        bullet("Fix L5 (per-phase timeouts) — wire clingo <tt>timeout</tt> argument."),
        h2("Medium-term"),
        bullet(
            "Scalability analysis: vary example count and background size; "
            "measure per-phase runtime scaling to identify bottlenecks."
        ),
        bullet(
            "Comparative experiment: run shared tasks in Aleph and ILASP; "
            "compare runtime, hypothesis quality, and intermediate artefact quality."
        ),
        bullet(
            "Bounded recursive deduction: investigate strategies allowing a "
            "constrained class of recursive modeb predicates."
        ),
        h2("Long-term"),
        bullet(
            "Neuro-symbolic integration: use neural perception to generate ground "
            "facts; XHAIL learns symbolic rules over neural features."
        ),
        bullet(
            "Noisy examples: extend abduction to tolerate inconsistent examples "
            "via weighted or majority-voting coverage."
        ),
        bullet(
            "LLM-assisted rule synthesis: use a language model to propose "
    ...
    story += [
        ...
        rule(),
        p(
            "XHAIL's three-phase pipeline provides a principled and transparent "
        ...
        p(
            "This reimplementation demonstrates the paradigm is viable in modern "
        ...
        p("The project is openly available at <b>github.com/everettmakes/xhail</b>."),
        sp(),
    ]
    ...
    story += [
        ...
        rule(),
        p(
            "Gebser, M., Kaminski, R., Kaufmann, B., &amp; Schaub, T. (2014). "
            "<i>Clingo = ASP + control.</i> arXiv:1405.3694.",
            "ref",
        ),
        p(
            "Law, M., Russo, A., &amp; Broda, K. (2014). Inductive learning of "
            "answer set programs. <i>ECAI 2014</i>, 311&#8211;316.",
            "ref",
        ),
        p(
            "Law, M., Russo, A., &amp; Broda, K. (2020). The ILASP system for "
            "inductive learning of answer set programs. "
            "<i>TPLP</i>, 20(2), 222&#8211;256.",
            "ref",
        ),
        p(
            "Law, M., Russo, A., Bertrand, J., &amp; Broda, K. (2020). FastLAS: "
            "Scalable ILP incorporating domain-specific optimisation criteria. "
            "<i>AAAI 2020</i>, 2877&#8211;2885.",
            "ref",
        ),
        p(
            "Muggleton, S. (1991). Inductive logic programming. "
            "<i>New Generation Computing</i>, 8(4), 295&#8211;318.",
            "ref",
        ),
        p(
            "Muggleton, S., Lin, D., Pahlavi, N., &amp; Tamaddoni-Nezhad, A. "
            "(2015). Meta-interpretive learning of higher-order dyadic datalog. "
            "<i>Machine Learning</i>, 100(1), 49&#8211;73.",
            "ref",
        ),
        p(
            "Ray, O. (2009). Nonmonotonic abductive inductive learning. "
            "<i>Journal of Applied Logic</i>, 7(3), 329&#8211;340.",
            "ref",
        ),
